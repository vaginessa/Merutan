...
if($_POST) {
    ...
    $to_Email = "samanyugarg@gmail.com"; // Write your email here
    ...
    if(strlen($_POST["userMessage"])<20) {
        $output = json_encode(array('type'=>'error', 'text' => '<i class="icon ion-close-round"></i> Message too short! Take your time and write a few words.'));
        die($output);
    }
    ...
    $emailcontent = 'Hey! You have received a new alert from one visitor, regarding a 404 error page not found.<br/><br/>'. "\r\n" .
                'His message: <br/> <em>'.$_POST["userMessage"].'</em><br/><br/>'. "\r\n" .
                '<strong>Feel free to contact him via email at : '.$_POST["userEmail"].'</strong>' . "\r\n" ;
    ...
    if(!$Mailsending) {
        ...
        $output = json_encode(array('type'=>'error', 'text' => '<i class="icon ion-close-round"></i> Oops! Looks like something went wrong...'));
        die($output);
        
    } else {
        $output = json_encode(array('type'=>'message', 'text' => '<i class="icon ion-checkmark-round"></i> Hello, your message has been sent, I will get back to you asap !'));
        die($output);
    }
}
?>